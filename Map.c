...
static int interestCity(LocationID currCity, LocationID *cities, int numCities);
...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
    ...
    if(g->connections[start] != NULL)
    {
        ...
        while (n != NULL) {
            ...
            if(n->v==end)
            {
                type[total_conn]=n->type;
                total_conn++;
                
            }
            /* else if(n->type==BOAT)
             {
             //  printf("checking SEA connection from %d\n",n->v);
             VList nn=g->connections[n->v];
    ...
    return total_conn;  // to keep the compiler happy
}



int findPathDist(Map g, LocationID start, LocationID end){
    ...
    int visited[NUM_MAP_LOCATIONS]={FALSE};
    int priorVisit[NUM_MAP_LOCATIONS]={[0 ... (NUM_MAP_LOCATIONS-1)] = UNKNOWN_LOCATION};
    
    //printf("Inside findPathDist, start %s, end %s\n", idToName(start),idToName(end));
    
    while(!emptyQueue(q) && index != end){
        
        visiting=leaveQueue(q);
        //  printf("Visiting %s\n", idToName(visiting));
        // while(index < numV(g)){
        if(g->connections[visiting] != NULL)
        {
            VList n = g->connections[visiting];
            while (n != NULL) {
                index=n->v;
                if(visited[index]!=TRUE && index != end){
                    enterQueue(q,index);
                    priorVisit[index]=visiting;
                    visited[index]=TRUE;
                    //   printf("Entered Q %s prior %s visited %d\n",idToName(index),idToName(visiting),TRUE);
...
//checks if the given 'currCity' is the part of the list of given cities
static int interestCity(LocationID currCity, LocationID *cities, int numCities) {
    int i = 0;
    int match = FALSE;
...
//Given closest city to a target city from a given list of possible cities

LocationID closestCity(Map g, LocationID target, LocationID *cities, int numCities) {
    assert(g != NULL);
    
    assert(target >= MIN_MAP_LOCATION && target <= MAX_MAP_LOCATION);
    ...
    LocationID currParent = target;
    enterQueue(searching, target);
    VList currCity;
    int reachedFinish = FALSE;
    LocationID closestCity = -1;
    
    // BFS from target
    while(!emptyQueue(searching) && !reachedFinish) {
        currParent = leaveQueue(searching);
        currCity = g->connections[currParent];
        
        while(currCity != NULL && !reachedFinish) {
            // If not observed previously, add to queue
            if(viewed[currCity->v] == -1) {
                enterQueue(searching, currCity->v);
                viewed[currCity->v] = currParent;
                
                // If city is a destination
                if(interestCity(currCity->v, cities, numCities) == TRUE) {
                    reachedFinish = TRUE;
                    closestCity = currCity->v;
                }
            }
